...
class ComplaintsPage(ctk.CTkFrame):
    ...
    def open_settings(self):
        ...
        self.labelBanner.configure(text="Settings are not available yet.", fg_color=theme.INFO)
    ...
    def submit_complaint(self):
        ...
        if severity not in {"1", "2", "3"}:
            self.labelBanner.configure(text="Severity must be 1, 2, or 3.", fg_color=theme.DANGER)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
            return
        ...
        if inserted:
            self.labelBanner.configure(text="Complaint submitted successfully.", fg_color=theme.SUCCESS)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
            self.refresh()
        else:
            self.labelBanner.configure(text="Could not submit complaint. Check apartment number and active lease.", fg_color=theme.DANGER)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())

    def refresh(self):
        self.Entrycomplaint.delete(0, 'end')
        self.EntryAPID.delete(0, 'end')
        self.EntryComplaintDetails.delete("1.0", "end")
        self.severity.set("2")
    ...
    def create_form(self):
        ...
        self.labelFrame = ctk.CTkFrame(content_card, fg_color=theme.TITLE)
        self.labelFrame.grid(row=6, column=0, columnspan=2, sticky="ew", padx=15, pady=10)
