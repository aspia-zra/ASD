...
class PaymentPage(ctk.CTkFrame):
    ...
    def __init__(self, parent, controller=None):
        ...
        self.alert_label = ctk.CTkLabel(
            self.content,
            text="Warning: There are overdue invoices! Please review.",
        ...
        self.labelFrame = ctk.CTkFrame(self.content, fg_color=theme.TITLE)
        self.labelFrame.grid(row=4, column=0, columnspan=2, sticky="ew", padx=15, pady=10)
    ...
    def mark_as_paid(self):
        ...
        if not selected:
            self.labelBanner.configure(text="Please select an invoice first.", fg_color=theme.WARNING)
        ...
        if status == "paid":
            self.labelBanner.configure(text="This invoice is already paid.", fg_color=theme.INFO)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
            return
        try:
            ...
            self.labelBanner.configure(text=f"Invoice {invoice_id} marked as paid.", fg_color=theme.SUCCESS)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())

            self.load_invoices()
        except Exception as e:
            self.labelBanner.configure(text="Could not update invoice.", fg_color=theme.DANGER)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
    ...
    def generate_receipt(self):
        ...
        try:
            ...
            self.labelBanner.configure(text="Could not generate receipt.", fg_color=theme.DANGER)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
    ...
    def reset_invoice(self):
        ...
        try:
            ...
            self.labelBanner.configure(text=f"Invoice {invoice_id} reset to pending", fg_color=theme.PRIMARY_LIGHT)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())

            self.load_invoices()
        except Exception as e:
            self.labelBanner.configure(text="Error resetting invoice:", fg_color=theme.DANGER)
            self.labelFrame.grid()
            self.after(3000, lambda: self.labelFrame.grid_remove())
