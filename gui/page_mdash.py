...
class DashboardPage(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.labelFrame = ctk.CTkFrame(self.main_container, fg_color=theme.TITLE)
        self.labelFrame.grid(row=1, column=0, columnspan=2, sticky="ew", padx=15, pady=10)
    ...
    def _open_complete_dialog(self, request):
        ...
        def submit_completion():
            ...
            if not resolution:
                self.labelBanner.configure(text="Please enter a resolution.", fg_color=theme.DANGER)
            ...
            if cost_text:
                try:
                    ...
                    self.labelBanner.configure(text="Cost must be a number.", fg_color=theme.DANGER)
                    self.labelFrame.grid()
                    self.after(3000, lambda: self.labelFrame.grid_remove())
                    return
            ...
            if request["type"] == "repair":
                if not time_text:
                    ...
                    try:
                        ...
                        self.labelBanner.configure(text="Time taken must be a whole number.", fg_color=theme.DANGER)
                        self.labelFrame.grid()
                        self.after(3000, lambda: self.labelFrame.grid_remove())
                        return
            ...
            def finalize_submit():
                self._build()
                if sent:
                    self.labelBanner.configure(text="Notification sent.", fg_color=theme.INFO)
                    self.labelFrame.grid()
                    self.after(3000, lambda: self.labelFrame.grid_remove())
                else:
